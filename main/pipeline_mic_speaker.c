...
void app_main()
{
    ...
    audio_element_handle_t wav_fatfs_stream_writer, i2s_stream_reader, i2s_stream_writer, wav_encoder, wav_decoder, amr_fatfs_stream_writer, amr_encoder;
    ...
    ESP_LOGI(TAG, "[2.0] Create audio pipeline_wav for recording");
    ...
    wav_encoder = wav_encoder_init(&wav_enc_cfg);
    ...
    ESP_LOGI(TAG, "[3.3] Create wav decoder to encode wav format");
    wav_decoder_cfg_t wav_dec_cfg = DEFAULT_WAV_DECODER_CONFIG();
    wav_decoder = wav_decoder_init(&wav_dec_cfg);

    ESP_LOGI(TAG, "[3.4] Create audio playback stream");
    ...
    // audio_element_setinfo(wav_dec_cfg, &info);

    ESP_LOGI(TAG, "[3.4] Register all elements to audio pipeline");
    audio_pipeline_register(pipeline_wav, i2s_stream_writer, "i2s_writer");
    audio_pipeline_register(pipeline_wav, i2s_stream_reader, "i2s_reader");
    audio_pipeline_register(pipeline_wav, wav_encoder, "wav_encoder");
    audio_pipeline_register(pipeline_wav, wav_decoder, "wav_decoder");
    // audio_pipeline_register(pipeline_wav, wav_fatfs_stream_writer, "wav_file");
    

    ESP_LOGI(TAG, "[3.5] Link it together [codec_chip]-->i2s_read_stream-->wav_encoder-->wav_decoder-->i2s_write_steam->[speaker]");
    ...
    audio_pipeline_unregister_more(pipeline_wav, i2s_stream_reader, i2s_stream_writer,
                                   wav_encoder, wav_decoder, NULL);
